param (
    [Parameter(HelpMessage = 'If you are initialising this file or want to force overwrite the persistent export data use this flag.')]
    [switch]$ForcePersist
)
<#
...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'InvokeADCheck'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/sensepost/InvokeADCheck'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) SensePost. All rights reserved."
        ModuleLicenseURI = 'https://github.com/sensepost/InvokeADCheck/LICENSE.txt'
        ModuleTags = 'ActiveDirectory' -split ','
        ModuleAuthor = 'Orange Cyberdefense'
        ModuleDescription = 'A PowerShell module designed to evaluate the security of Active Directory environments'
        ...
        OptionSensitiveTermsInitialized = $True
        ...
        # You should NOT enter this key here but rather manually enter it in the InvokeADCheck.buildenvironment.json file with: Set-MBBuildEnvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "InvokeADCheck.buildenvironment.json"
